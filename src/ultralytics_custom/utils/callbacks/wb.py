# updated `on_fit_epoch_end`
# add `wb.alert` in `on_train_end`
...
def _log_train_data_num(train_dir_list):
    if isinstance(train_dir_list, str):
        train_dir_list = [train_dir_list]
    label_dir_list = [os.path.join(Path(x).parent, "labels") for x in train_dir_list]
...
def _log_class_num(train_dir_list, categories):
    ...
    cnt_dict = {x: 0 for x in categories.values()}
    for label_dir in label_dir_list:
        for filename in os.listdir(label_dir):
            with open(os.path.join(label_dir, filename), "r") as f:
                label_list = [x.strip() for x in f.readlines()]
            for label in label_list:
                cnt_dict[categories[int(label[0])]] += 1
    ...
    for k, v in cnt_dict.items():
        wb.run.log({f"class_num/{k}": v}, commit=False)
...
def on_pretrain_routine_start(trainer):
    ...
    # log class num
    _log_train_data_num(trainer.data["train"])
    _log_class_num(trainer.data["train"], trainer.data["names"])
...
def on_fit_epoch_end(trainer):
    ...
    for i, c in enumerate(trainer.validator.metrics.ap_class_index):
        class_wise_metrics[f"class_metrics_AP50/{trainer.validator.names[c]}"] = (
            trainer.validator.metrics.class_result(i)[-2]
        )
    ...
    if "val_json" in trainer.data:
        ...
        for pred in trainer.validator.jdict:
            ...
            if pred["category_id"] in opt_conf_thresh_dict and pred["score"] < opt_conf_thresh_dict[pred["category_id"]]:
                continue
